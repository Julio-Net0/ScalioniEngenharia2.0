...
export function CheckoutDialog({ plantaId, titulo, preco }: Props) {
    ...
    const { criarPedidoUseCase } = useDependencies()
    ...
    async function onSubmit(data: CheckoutValues) {
        setLoading(true)
        try {
            const res = await criarPedidoUseCase.execute(
                plantaId,
                data.nome,
                data.email,
                data.telefone
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="w-full max-w-md bg-card-bg border border-primary/30 p-8 z-50 shadow-2xl">
                <DialogHeader>
